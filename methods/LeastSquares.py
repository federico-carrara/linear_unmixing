...
def lstsq_fit_2D(mixed_img, fp_ref_matrix):
    ...
    for r in range(n_rows):
        for c in range(n_cols):
            ...
            fp_conc, residuals, rank, sing_vals = lstsq(a=fp_ref_matrix, b=mixed_pixel)
    ...
    for z in tqdm(range(nz), desc="Solving LS on z slice"):
        for y in range(ny):
            for x in range(nx):
                ...
                fp_conc, residuals, rank, sing_vals = lstsq(a=fp_ref_matrix, b=mixed_voxel)
